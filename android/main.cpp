...
{ Q_OBJECT
	...
	private:
		...
		void visibleArea (double *x0, double *y0, double *x1, double *y1)
		{
			...
			// Карта долготу не нормирует, а копит при прокрутке: после
			// долгого хода на восток участок приходит как 350..370, и
			// обе стороны — и плитки, и NOAA — понимают его неправильно.
			// Приводим начало к обычному диапазону, ширину сохраняя.
			double w = *x1 - *x0;
			while (*x0 >= 180.0)  *x0 -= 360.0;
			while (*x0 < -180.0)  *x0 += 360.0;
			*x1 = *x0 + qMin (360.0, w);
		}
